...
NODES = {
    "manager-node" => { hostname: "manager", ip: "192.168.56.11", memory: 1024, cpus: 1 },
    "worker-node" => { hostname: "worker", ip: "192.168.56.12", memory: 1024, cpus: 1 },
}
            ...
            if name == "worker-node"
                ...
                node.trigger.after :up do |trigger|
                    trigger.ruby do |env, machine|
                        ...
                        system("vagrant ssh worker-node -c \"sudo docker info 2>/dev/null | grep -q 'Swarm: active' || sudo docker swarm join --token #{token} #{MANAGER_IP}:2377\"")
                    end
                end
            end
        end
